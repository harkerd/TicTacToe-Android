...
public class Draw3DView extends View {
    ...
    public Draw3DView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public Draw3DView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for(int i = 0; i < 3; i++)
        {
            for(int j = 0; j < 3; j++)
            {
                if(grid.getAt(i, j, axis) == Grid3DModel.X)
                {
                    drawX(canvas, widthPoints[i] + letter_buffer, heightPoints[j] + letter_buffer);
                }
                else if(grid.getAt(i, j, axis) == Grid3DModel.O)
                {
    ...
    public void redraw(Grid3DModel grid, AxisView axis) {
        this.grid = grid;
        this.axis = axis;
        this.invalidate();
    }
